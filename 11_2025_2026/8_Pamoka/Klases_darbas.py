...
biblical_animals = [
    ...
    "Wolf",
    "Zebra",
    "Unicorn"  # Note: Unicorn is referenced symbolically in biblical texts
]
#Duotas atskiras tyrųjų gyvūnų sąrašas
pure_biblical_animals = [
    "Cattle",
    "Camel",
    "Ewe",
    "Eagle",
...
#Patikrinkite ar gyvūnas sąraše biblical_animals yra tyras, jei taip pakeiskite visas raides į didžiąsias ir užrašykite atbulai
#1 Užduotis
...
for i in range(len(biblical_animals)):
    for j in range(len(pure_biblical_animals)):
        if biblical_animals[i] == pure_biblical_animals[j]:
            biblical_animals[i] = biblical_animals[i][::-1].upper()
print(biblical_animals)
